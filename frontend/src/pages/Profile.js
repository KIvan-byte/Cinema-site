...
const Profile = () => {
  ...
  useEffect(() => {
    if (location.state?.paymentSuccess) {
      setSuccessMessage(t("profile.paymentSuccess"));
    }
    const fetchReservations = async () => {
      try {
        ...
        setError(t("profile.fetchError") || 'Failed to load your reservations');
        setLoading(false);
      }
    };
    fetchReservations();
  ...
  const handleDownloadTicket = async (reservationId) => {
    try {
      const response = await api.get(`/reservations/${reservationId}/ticket`, { responseType: 'blob' });
      ...
      alert(t("profile.ticketDownloadError"));
    }
  };
  
  if (loading) {
    return <div className="flex justify-center items-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-cinema-red"></div></div>;
  }
  
  return (
    <div className="container mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold mb-6">{t("profile.myProfile")}</h1>
      {successMessage && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg mb-6">{successMessage}</div>}
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-cinema-red to-cinema-red-dark text-white p-5">
          <h2 className="text-xl font-semibold">{t("profile.myTickets")}</h2>
        </div>
        
        {error && <div className="p-4 text-cinema-red-dark">{error}</div>}
        
        {reservations.length === 0 ? (
          <div className="p-6 text-cinema-gray">{t("profile.noTickets")}</div>
        ) : (
